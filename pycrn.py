        ...
        for op in operations:
            ...
            for operand in op[1:]:
                ...
                if isinstance(operand, tuple) and len(operand) == 2 and isinstance(operand[0], str):
                    ...
                    if val < 0: 
                        # CHECK as reaction network can only take positive input Need a flag if user want to deactivate this
                        raise ValueError(f"Initial concentration for '{name}' must be non-negative, but got {val}")
                        
                    # (do not overwrite existing)
                    if name not in self.initials:
                        self.initials[name] = val
                    # ensure species set has the name (strings only)
                    self.species.add(name)
                    new_args.append(name)
                else:
                    # regular operand (string species name)
                    new_args.append(operand)
                    # if it's a string, add to species set now (safe)
                    if isinstance(operand, str):
                        self.species.add(operand)

            # reconstruct op as tuple with op_name + string args
            processed_ops.append(tuple([op_name] + new_args))

        # store processed phase operations
        self.phases.append(processed_ops)

        # also process inputs dict if provided (without overwriting existing initials)
        if inputs:
